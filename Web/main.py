...
def hospital(token):
    ...
    places = [place.name for place in query_result.places]
...
def symptoms():
    ...
    sym = cursor.fetchall()
    sym1 = list(sym)
    symptoms = [item[1] for item in sym1]
...
@app.route('/')
def index():
    return render_template('index.html') if 'loggedin' not in session else home()
...
def diagnosefinal():
    ...
    if 'loggedin' in session:
        ...
        if (request.method == 'POST'):
            ...
            filename = 'finalized_model.sav'
            feature_dict = {f: i for i, f in enumerate(features)}
            l = [request.form[f'sym{str(i)}'] for i in range(n)]
            for i in l:
                s=i
                m=feature_dict[s]
                if (m!=0):
...
def diagnosedetails():
    ...
    if 'loggedin' in session:
        ...
        if feel := request.form['feel']:
            data = [feel]
            cv = pickle.load(open("vectorizer.pickle", 'rb'))     #Load vectorizer
            loaded_model = pickle.load(open('disease_predict.sav', 'rb'))
            vect=cv.transform(data).toarray()
            p=loaded_model.predict(vect)
